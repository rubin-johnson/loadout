...
def test_write_creates_target_dir(tmp_path):
    nested = tmp_path / "a" / "b"
    write_state(nested, {"active": "x", "applied_at": "t", "package_path": "p", "manifest_version": "1", "backup": "b"})
    assert read_state(nested) is not None


def test_state_file_is_named_correctly(tmp_path):
    write_state(
        tmp_path, {"active": "x", "applied_at": "t", "package_path": "p", "manifest_version": "1", "backup": "b"}
    )
